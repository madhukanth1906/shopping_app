...
# Fix the missed localizations in shop.html
shop_replacements = {
    'The Alabaster Slip': 'Banarasi Silk Saree',
    'Midi Linen Drape': 'Pastel Embroidered Lehenga',
    'Terra Pleated Column': 'Chanderi Kurta Set',
    'The Eyelet Maxi': 'Georgette Anarkali',
    'Oversized Dune Shirt': 'Kanjeevaram Saree',
    'Nightfall Lace Gown': 'Velvet Lehenga',
}

shop_html = 'shop.html'
if os.path.exists(shop_html):
    with open(shop_html, 'r', encoding='utf-8') as f:
        content = f.read()
    for old, new in shop_replacements.items():
        content = content.replace(old, new)
    with open(shop_html, 'w', encoding='utf-8') as f:
        f.write(content)
...
with open('public/catalog.js', 'w', encoding='utf-8') as f:
    f.write(f"window.ATELIER_CATALOG = {json.dumps(catalog, indent=4)};\n")
print("catalog.js created.")

# Now we need to modify index.html and shop.html to link to these.
# In index.html: <div class="min-w-[320px] snap-start group relative">
def add_links_index(content):
    # We will just inject onclick into the product div.
    # It's tricky with regex if we don't know the exact order.
    # But we can find the <h3> and use it to figure out the ID.
    for pid, pdata in catalog.items():
        name = pdata['name']
        # Find the div block containing this name
        # We can just replace `<div class="min-w-[320px] snap-start group relative">` that precedes this name
        pattern = r'(<div class="min-w-\[320px\] snap-start group relative">)([^<]*<div[^>]*>[\s\S]*?<h3[^>]*>' + re.escape(name) + '</h3>)'
        content = re.sub(pattern, f'<div class="min-w-[320px] snap-start group relative cursor-pointer" onclick="window.location.href=\'/product.html?id={pid}\'">\\2', content)
    return content

if os.path.exists('index.html'):
    with open('index.html', 'r', encoding='utf-8') as f:
        content = f.read()
    content = add_links_index(content)
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(content)

def add_links_shop(content):
    for pid, pdata in catalog.items():
        name = pdata['name']
        pattern = r'(<div class="product-card group cursor-pointer">)([^<]*<div[^>]*>[\s\S]*?<h3[^>]*>' + re.escape(name) + '</h3>)'
        content = re.sub(pattern, f'<div class="product-card group cursor-pointer" onclick="window.location.href=\'/product.html?id={pid}\'">\\2', content)
    return content

if os.path.exists('shop.html'):
    with open('shop.html', 'r', encoding='utf-8') as f:
        content = f.read()
    content = add_links_shop(content)
    with open('shop.html', 'w', encoding='utf-8') as f:
        f.write(content)

print("index.html and shop.html updated with product links.")
